...
salesByWeek.forEach(week => {
    const name = week.sales_agent.first_name + " " + week.sales_agent.last_name
    const vehicle = week.vehicle
    const profit = week.gross_profit
    container.innerHTML += `<hr><h3>${name}</h3>`
    for (const spec of Object.entries(vehicle)) {
        container.innerHTML += `<p>${spec[0]}: ${spec[1]}`
    }
    container.innerHTML += `<h4>$${profit}</h4>`
});
